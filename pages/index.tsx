    ...
    <div className="flex flex-col gap-8 p-2 mx-auto max-w-5xl w-full">
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="flex flex-col sm:flex-row gap-2">
          ...
          <div className="rounded-xl border border-gray-300 p-2 bg-white flex-1">
            <h4 className="text-center mb-2">Class</h4>
            <div className="flex capitalize gap-2 flex-col sm:flex-row sm:justify-center">
...
export const getServerSideProps = async () => {
  ...
  const highestAverageDmg = await client.query(
    "select A.player_name,A.player_class,A.damage as value from (select player_name,player_class,avg(damage) as damage,count(*) from stats left join match on stats.match_id=match.match_id where match_type='challenge' group by player_name,player_class having count(*)>=$1) as A order by damage desc limit 3;",
    [MIN_GAMES_PLAYED]
  );
  const highestAverageKills = await client.query(
    "select A.player_name,A.player_class,A.kills as value from (select player_name,player_class,avg(kills) as kills,count(*) from stats left join match on stats.match_id=match.match_id where match_type='challenge' group by player_name,player_class having count(*)>=$1) as A order by kills desc limit 3;",
    [MIN_GAMES_PLAYED]
  );
  const highestAverageHealing = await client.query(
    "select A.player_name,A.player_class,A.healing as value from (select player_name,player_class,avg(healing) as healing,count(*) from stats left join match on stats.match_id=match.match_id where match_type='challenge'  group by player_name,player_class having count(*)>=$1) as A order by healing desc limit 3;",
    [MIN_GAMES_PLAYED]
  );
