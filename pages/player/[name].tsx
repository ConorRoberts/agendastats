          ...
          {Object.entries(records as object)
            ...
            .map(([label, { player_class, value }]) => (
              <div key={label} className="flex-1">
                <h4>{label}</h4>
                <div className="shadow-md capitalize rounded-xl p-2 border border-gray-200">
                  <p>{player_class}</p>
                  <p>{value}</p>
                </div>
              </div>
          ...
          {Object.entries(averages).map(([label, data]) => (
            <div key={label} className="flex-1 flex flex-col gap-4">
              ...
              <div className="flex flex-row gap-4">
                {Object.entries(data)
                  .filter(([_stat, { value }]) => value > 0)
                  .map(([stat, { player_name, value }]) => (
                    <div
                      key={label + player_name}
                      className="shadow-md capitalize rounded-xl p-2 border border-gray-200 flex-1"
                    >
                      <p>{stat}</p>
...
export const getServerSideProps = async (context: NextPageContext) => {
  ...
  const averageKills = await client.query(
